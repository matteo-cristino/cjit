...
#ifdef LIBC_MINGW32
typedef struct {
    int (*function)(int, char**); // Pointer to the function
    int argc;
    char **argv;
} ThreadData;

// Thread function to call the dynamically loaded function
DWORD WINAPI thread_func(LPVOID lpParam) {
    ThreadData *data = (ThreadData*)lpParam;
    return data->function(data->argc, data->argv);
}
...
static int cjit_exec_win(TCCState *TCC, const char *ep, int argc, char **argv) {
    ...
    // Create the data to pass to the thread
    ThreadData data;
    data.function = _ep;
    data.argc = argc;
    data.argv = argv;
    ...
    if (!GetExitCodeThread(thread, &exitCode)) {
        _err("Failed to get thread exit code");
        res = -1;
    } else {
        res = (int)exitCode;
        _err("Thread has returned %d", res);
    }
